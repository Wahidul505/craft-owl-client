...
const CheapestTool = () => {
  ...
  useEffect(() => {
    fetch("https://craft-owl-server.vercel.app/cheapest-tool")
      .then((res) => res.json())
      .then((data) => setCheapTool(data[0]));
  }, []);
  return (
    <div>
      <h1 className="text-center text-primary text-4xl mb-6">
        Cheapest Product On Store
      </h1>
      <div
        className="hero h-72 bg-cover bg-center bg-no-repeat"
