...
const DeleteToolModal = ({ deletingTool, setDeletingTool, refetch }) => {
  ...
  const handleDeleteTool = (id) => {
    fetch(`https://craft-owl-server.vercel.app/tool/${id}`, {
      method: "DELETE",
      headers: {
    ...
    <div>
      ...
      <div className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <p>
            With Quantity of:{" "}
            <span className="text-error">{availableQuantity}</span>
          </p>
          <p>
            And Price: <span className="text-error">${price}</span>
          </p>
          <div className="modal-action">
            ...
            <label for="delete-tool-modal" className="btn">
              Cancel
            </label>
          </div>
        </div>
