...
const MyProfile = () => {
  ...
  const {
    ...
    fetch(`https://craft-owl-server.vercel.app/user/${user?.email}`, {
      headers: {
      ...
      <div className="card bg-primary text-primary-content -mx-8 md:mx-0">
        <div className="card-body -mx-4 md:mx-0">
          <div className="avatar indicator">
            {role === "admin" && (
              <span className="indicator-item badge badge-accent">Admin</span>
            )}
            <h2 className="card-title text-3xl mt-2 mr-8">
              {user?.displayName || ""}
            </h2>
          </div>
          <p>{user?.email || ""}</p>
          {updateStatus && (
            <div>
              ...
              <p>
                LinkedIn Profile: <br />{" "}
                <a className="text-white font-semibold" href={linkedin}>
                  {linkedin || ""}
                </a>
              </p>
            </div>
